...
// async function configureBrowser (newurl){
...
async function configureBrowser(newurl) {
    const browser = await puppeteer.launch({ headless: true }); // Set headless to true for headless mode, or false for non-headless mode
    const page = await browser.newPage();
    await page.goto(newurl);
    const html = await page.content();
    await browser.close();
